...
aws --endpoint-url="${AWS_ENDPOINT}" --region "${AWS_DEFAULT_REGION}" sns create-topic --name "modular-monolith-topic.fifo" --attributes FifoTopic=true,ContentBasedDeduplication=true
...
aws --endpoint-url="${AWS_ENDPOINT}" --region "${AWS_DEFAULT_REGION}" sns subscribe --topic-arn "arn:aws:sns:${AWS_DEFAULT_REGION}:000000000000:modular-monolith-topic.fifo" --protocol sqs --notification-endpoint "arn:aws:sqs:${AWS_DEFAULT_REGION}:000000000000:modular-monolith-queue.fifo" --attributes RawMessageDelivery=true
